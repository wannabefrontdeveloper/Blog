...
class MongoClient extends mongo_types_1.TypedEventEmitter {
    ...
    async _connect() {
        ...
        if (options.credentials && options.credentials.mechanism === providers_1.AuthMechanism.MONGODB_OIDC) {
            let allowedHosts = mongo_credentials_1.DEFAULT_ALLOWED_HOSTS;
            if (options.credentials.mechanismProperties && options.credentials.mechanismProperties.ALLOWED_HOSTS) {
                allowedHosts = options.credentials.mechanismProperties.ALLOWED_HOSTS;
            }
        
            let isServiceAuth = false;
            if (options.credentials.mechanismProperties && options.credentials.mechanismProperties.PROVIDER_NAME) {
                isServiceAuth = true;
            }
        ...
        const topologyConnect = async () => {
            try {
                await new Promise((resolve, reject) => {
                    if (this.topology) {
                        this.topology.connect(options, err => {
                            if (err) {
                                reject(err);
                            } else {
                                resolve();
                ...
                if (this.topology !== undefined && this.topology !== null) {
