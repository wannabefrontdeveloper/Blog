...
class MongoClient extends mongo_types_1.TypedEventEmitter {
    ...
    async _connect() {
        ...
        if (options.tls) {
            if (typeof options.tlsCAFile === 'string' && !options.ca) {
                options.ca = await fs_1.promises.readFile(options.tlsCAFile);
            }
            if (typeof options.tlsCRLFile === 'string' && !options.crl) {
                options.crl = await fs_1.promises.readFile(options.tlsCRLFile);
            }
            if (typeof options.tlsCertificateKeyFile === 'string' && (!options.key || !options.cert)) {
        ...
        if (options.credentials && options.credentials.mechanism === providers_1.AuthMechanism.MONGODB_OIDC) {
            const mechanismProperties = options.credentials && options.credentials.mechanismProperties;
            const allowedHosts = mechanismProperties && mechanismProperties.ALLOWED_HOSTS || mongo_credentials_1.DEFAULT_ALLOWED_HOSTS;
            const isServiceAuth = !!(mechanismProperties && mechanismProperties.PROVIDER_NAME);
        ...
        const topologyConnect = async () => {
            try {
                await (0, util_1.promisify)(callback => this.topology && this.topology.connect(options, callback))();

            }
            catch (error) {
                this.topology && this.topology.close({ force: true });

                throw error;
            }
        };
        if (this.autoEncrypter) {
            this.autoEncrypter && (await this.autoEncrypter.init());
